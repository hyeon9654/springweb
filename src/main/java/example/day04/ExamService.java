...
public class ExamService {
    ...
    public boolean 삭제( int no ){
        ...
        // 반환타입 : 없음 , 삭제여부 findXXX 이동하여 확인
        examRepository.deleteById(no);
        return true; // 삭제 성공
    }

    // [4] 수정
    @Transactional // 트랜잭션이란? 여러개 SLQ 하나의 단위로 묶음
    // 만약에 여러개 SQL중에 하나라도 SQL오류이면 전체 ROLLVACK(취소) 모두 성공하면 COMMIT(완료)
    // 활용처: 계좌이체(출금/입금), 회원가입포인트(회원가입/최소포인트지급): 2개 이상 기능을 하나로 묶음
    public boolean 수정( ExamEntity entity ){
        // 1. 영숙된 엔티티 조회[PK: 수정할 번호]
